...
- ESJ-曉朔國度 → 小说分类 → 小说列表 → 小说详情。五个分类的两段式 `/forum/{categoryId}/{novelId}/` 页面属于站点旧架构；客户端只读取其标题区域中的 `/detail/{novelId}.html`，随后用小说详情页替换当前导航项，不再把旧页面或其中的主题表暴露为额外层级。
...
因此，用户界面中的 ESJ 路径止于第三级小说详情；天空大公國仍保留分类、讨论板块、主题/帖子层级。只有已知的五个 ESJ 分类可以把第二段 ID 按小说旧入口处理，天空大公國的第二段 ID 仍是讨论板 ID。
...
子板块 `/forum/{categoryId}/{boardId}/` 使用 `#dataTable[data-url]` 的 Bootstrap Table 壳。页面初始 HTML 可能只有 `no-records-found` 占位行，即使 `data-url` 的 `totalRows` 大于 0；等待站点脚本后可看到主题行。`totalRows=0` 才表示已确认的合法空板块。主题接口请求前，站点脚本会向当前板块页 POST `plxf=getAuthToken`，再将响应中的令牌放入 `Authorization` 请求头；本次客户端缺少该认证头与接口返回业务状态 `301` 相符。客户端必须使用当前板块 URL 获取令牌，收到该状态时重新获取并最多重试一次。
